...
> JSON/config-driven statistical dashboard platform. Phase 1: dashboard (JSON config → renderer). Phase 2: Constructor (generates JSON, no code). Per-module laws: `engine/CLAUDE.md`, `apps/plugins/CLAUDE.md`. Enforcement rules: `.claude/rules/`.
...
3. **Dependency arrow (Clean Architecture):** `engine/core ← engine/react ← engine/plugins ← apps/geostat`. Never import against the arrow. `engine/react` stays app-agnostic (Geostat specifics → `engine/plugins/`).
