...
> JSON/config-driven statistical dashboard platform. Phase 1: dashboard (JSON config → renderer). Phase 2: Constructor (generates JSON, no code). Per-module laws: `platform/engine/CLAUDE.md`, `platform/engine/plugins/CLAUDE.md`. Enforcement rules: `.claude/rules/`.
...
3. **Dependency arrow (Clean Architecture):** `platform/engine/core ← platform/engine/react ← platform/engine/plugins ← platform/apps/geostat`. Never import against the arrow. `platform/engine/react` stays app-agnostic (Geostat specifics → `platform/engine/plugins/`).
