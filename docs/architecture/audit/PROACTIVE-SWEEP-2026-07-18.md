...
> **STATUS: BUILT-GREEN (2026-07-18, ADR-048, card 0092).** ONE fetch scheduler at the `ApiStore` network seam (`packages/core`, client-global) — concurrency cap + `Retry-After`-honoring backoff on 429/503 + stale-while-revalidate last-good; `transient-retrying` joins the `ValueState` honest-state grammar (bilingual kpi tile); the boot fetch backs off instead of the English `emptyManifest()` dead-end. Full suite 0 failed. Live-verify pending.
