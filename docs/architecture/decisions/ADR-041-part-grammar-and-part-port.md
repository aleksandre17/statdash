...
## Delta 3 (2026-07-12) — Phase 6 landed: wrapper/leaf derived, kind-as-containment retired (⚠️ the one-way step)

**Context.** Phase 6 of `PLAN-part-grammar-strangler-build.md` — the SOLE one-way `contract` step — landed. Wrapper/leaf becomes a DERIVED predicate and the last KIND/FLAG read used as a containment mechanism is retired. Machine-gated exactly like ADR-023 R2: landed only with `FF-DERIVED-CONTAINMENT` machine-green as a hard `[]` gate over EVERY registered META and the whole stored corpus.

**Delivered.**
- **The derived predicate (ROOT-2, one home).** `isWrapper(meta) = partFieldsOf(meta).length > 0` — WRAPPER ⇔ declares ≥1 part field; SINGLE ELEMENT ⇔ none. Added to `packages/react/src/engine/slice-meta.ts`, exported from the engine barrel. The owner's wrapper-vs-single-element intuition now has ONE home; kpi-strip is a WRAPPER (declares a `value` part) though its kind is a leaf-panel — kind and contract reconciled, never read as a containment signal.
- **The node-tree specialization.** `isNodeContainer(meta) = partFieldsOf(meta).some(p => p.residence === 'slot')` — a node accepts child NODES iff it declares a `slot` part. A `value`/`sourced` wrapper (kpi-strip items, filter-bar controls) is a wrapper-by-contract but NOT a node-tree drop target (its parts are values/projections, not draggable nodes). This is the precise correction the plan body's shorthand ("use `isWrapper`") required and the fence comment already specified ("`isWrapper` … restricted to slot residence"): the general `isWrapper` would have made kpi-strip/filter-bar node-drop targets — a regression `paletteContextual` catches.
- **The retired read.** `apps/panel/src/canvas/insertNode.ts::isDropTarget` no longer reads `canHaveChildren`; it derives from `isNodeContainer`. Byte-identical to the flag-read, proven corpus-wide by the plugins `FF-DERIVED-CONTAINMENT` semantic gate: `canHaveChildren === true ⟺ declares a slot part`. The app-side allowlist is struck to `[]` (BASELINE 0).
- **`sliceType` / `canHaveChildren` retained where they are real.** `sliceType` stays the palette / behaviour-routing discriminant (page/chrome/control identities); `PanelSliceMeta.canHaveChildren?: false` stays a compile-time refinement. Only their use as a *containment* signal is removed. The permanent permitted homes: `registerSlice.ts` (registry-view ingestion of the facet), `objectRegistry`/`slice-meta` (declaration), and the plugins `catalog.ts` palette-view (carries the facet into a `PaletteEntry`, like `StoredMeta` — never a containment decision).

**The BandDescriptor de-alias decision (Delta 3, residence-at-field contract).** The CANONICAL part declaration is the `sourced` `PartField` that `partFieldsOf` emits; **`BandDescriptor` is retained as a thin, documented NODE-LEVEL surface alias** that projects into it (`{ source }` → `{ field: source, residence: 'sourced', source }`). It is NOT physically relocated onto a field in this phase — the residence stays node-level (the ONE grandfathered exception `FF-RESIDENCE-AT-FIELD` allowlists, BASELINE 1). **Rationale:** moving the residence onto a real field (retiring the last node-level residence → `FF-RESIDENCE-AT-FIELD` `[]`) requires a NEW field-level sourced-declaration surface form plus a `filter-bar` META migration — a REVERSIBLE `expand` step deliberately kept OUT of the sole one-way containment step, per one-way-door hygiene (keep the irreversible step minimal). It is the recommended next `expand` phase; `FF-RESIDENCE-AT-FIELD` stays in guard mode at BASELINE 1 until then.

**FF changes.** `FF-DERIVED-CONTAINMENT` → **hard `[]` gate**: the engine tooth (no `.canHaveChildren` read outside the registry-view ingestion) and the app tooth (allowlist emptied, BASELINE 0) are both zero-tolerance; the plugins semantic tooth asserts non-contradiction via the shipped `isNodeContainer`. The engine + plugins fitness now test the SHIPPED `isWrapper`/`isNodeContainer` (local duplicates removed) — the gate asserts the real predicate, not a copy.

**Tooling fix (surfaced en route).** The `ADR041-part-grammar-no-bridge` check-laws tripwire (`.claude/project.json`, glob `platform/apps/panel/src/**`, whole-file scan) forbids tokens the FF suite itself must hold as BITES fixtures + regex constants (e.g. `registerPartSource('kpi-strip', src)`). A tripwire whose own `msg` names "SSOT = the FF suite" cannot gate its own authoritative guard — the identical category error `post-edit-laws.py` already avoids for `.claude/project.json`. Fixed at root: `post-edit-laws.py` now exempts `*.fitness.*` / `*.test.*` from the content `law_patterns` scan (production source is still fully covered; the FF suite + eslint remain SSOT).

**Reversibility.** NONE for the containment semantic — this is the one-way door (gate: `FF-DERIVED-CONTAINMENT` hard-`[]`, owner sign-off). The *additive* engine exports (`isWrapper`/`isNodeContainer`) are individually reversible; the behaviour is byte-identical to the retired flag-read, so rollback risk is nil.
