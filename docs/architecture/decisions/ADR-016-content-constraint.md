---
title: ContentConstraint (cube region — legal dimension-value combinations)
status: Accepted (implemented)
date: 2026-06-23
authors: architect / database-architect
migrated_from: project_content_constraint_adr (orig. ADR-0027, SDMX-P0-1)
---

# ADR-016 — ContentConstraint (cube region) [orig. ADR-0027]

**Status:** Accepted + implemented (2026-06-23; V26 + validate.ts/region.ts/types.ts + provisioning + fitness test). The last SDMX-P0 gap from the deep-research.

## Context

V4's `validate_observation_dim_key` validates each dim_key value in isolation (value ∈ classifier), but NOT combinations. ACCOUNTS_SEQUENCE needs combination rules (e.g. account `B9` is legal ONLY on side `U` by SNA design); the Cartesian product contains SNA-illegal combinations that currently pass the trigger and enter gold.

## Decision

- **A predicate-row ContentConstraint model:** `content_constraint` (header, role pinned to `allowed` — `actual` is unrepresentable in the table) + `content_constraint_member` (predicate rows, optionally scoped by a `when` condition) + a `dim_key_in_allowed_region()` helper + a `cube_actual_region` VIEW.
- **Validated in SILVER** (`ingest/region.ts` called by `validate.ts`), emitting `ILLEGAL_COMBINATION` only on structurally-sound rows.
- **Multiple conditions on the same (dim,code) are AND-conjoined** (fail-safe restrictive); the `cube_actual_region` view feeds the Constructor's cube profile.

## Rejected Alternatives

1. **A DB trigger on `stats.observation`** — REJECTED: the region check lives in the silver gate (in-memory per row, one region query), not a per-row trigger; keeps V1–V25 objects untouched and avoids trigger cost on the hot insert path.
2. **A rule-group table for true OR semantics** — REJECTED for now: AND-conjunction covers the real single-condition case with a defined fail-safe meaning; a nullable `rule_group_id` is the additive escalation if OR is ever needed (YAGNI).
3. **FK `member.code → stats.classifier`** — REJECTED: a constraint may be authored before its codelist (same posture as V11 silver); validity is the authoring path + fitness function's job.

## Consequences

- Positive: combination-legality is enforced in silver with one SSOT helper; the actual region is a view (no denormalization); re-provisioning is idempotent (UNIQUE NULLS NOT DISTINCT).
- Negative / cost: OR semantics require a future additive migration; member codes are not FK-guarded.
- Fitness: `content-constraint.fitness.test.ts` (no published obs out of region + predicate-teeth against the real B9 rule).

---

## Detailed Record (preserved verbatim from architect memory)

> Migrated from `.claude/agent-memory/architect/`. Part of the backend/DB ADR family (see also ADR-017, ADR-018, and the <8KB `project_bootstrap_runner_adr` pointer).
