---
title: Vintage-as-Release (real-time / vintage database) for SDMX-P0-2
status: Accepted (design only; migrations/code are the next step)
date: 2026-06-22
authors: architect / database-architect
migrated_from: project_vintage_release_adr (orig. ADR-0025, SDMX-P0-2)
---

# ADR-0025 — Vintage-as-Release (real-time / vintage database) [orig. ADR-0025]

**Status:** Accepted (design only, 2026-06-22; migrations/code not yet written). Backend/DB ADR family (see ADR-0023, ADR-016, ADR-017, ADR-018).

## Context

V8 captures revision PRE-IMAGES into `stats.observation_revision` keyed only by the server timestamp `revised_at`. Revisions are per-observation and NOT grouped into a release, so a coherent published vintage cannot be reconstructed. SDMX-P0-2 needs: (1) "GDP series AS IT WAS PUBLISHED on date D" (vintage reconstruction), and (2) revision triangles (how an estimate for period P evolved across releases).

## Decision

- **Release = a publication-event AGGREGATE** (SDMX/ECB sense), NOT 1:1 with `publishSubmission`. New table `stats.release` (UUID id, label JSONB, status open|published|superseded, published_at, nullable dataset_code, is_current via partial unique index). A named release bundles 1..N submissions.
- **Lifecycle: explicit open → attach submissions → publish.** A submission carries a nullable `release_id` FK; `publishSubmission` auto-opens a release when none is attached (single-submission path Just Works); publishing flips the prior current release to `superseded` and stamps `published_at`.
- **Stamping via GUC, read by triggers** (mirrors V8 `app.revised_by` / V17 `app.dry_run`): `SET LOCAL app.release_id`; a BEFORE trigger stamps `observation.release_id`; the V8 capture trigger is extended to stamp `superseded_by_release_id` and copy the old `release_id` into the pre-image as `set_by_release_id` — making each pre-image a release-keyed closed validity interval.
- **Validity interval lives on the pre-image, derived from release publish times;** the as-of query picks the current value if its `release.published_at <= D` with no superseding release ≤ D, else the pre-image whose `[set, supersede)` interval covers D.
- **dataset_version reconciliation:** a published release bumps `dataset_version` (reuse `stats.bump_dataset_version`); `release_id` is the durable vintage key, `dataset_version` stays the cheap monotonic ETag validator. NOT merged (SSOT each).

## Rejected Alternatives

1. **Full bitemporal SCD-2 on `observation` (valid_from/valid_to on the hot hypertable).** REJECTED: doubles the hottest table, fights TimescaleDB compression / partition-key unique constraints, and the V8 pre-image log ALREADY is the history store — reuse it, don't duplicate (SSOT).
2. **Release = 1:1 with submission.** REJECTED: a real publication event spans facts + codelists + displays (multiple submissions); ECB/IMF releases are named events, not file uploads.
3. **Reconstruct vintage purely from `revised_at` timestamps (no release).** REJECTED: server timestamps do not group a coherent published set; two figures published in one event get micro-different timestamps and a mid-event D could split a release. A release gives a single atomic `published_at` per vintage.

## Consequences

- Positive: coherent vintage reconstruction + revision triangles; reuses the V8 pre-image log (no cube duplication); single atomic `published_at` per vintage; curators can bundle multi-file releases.
- Negative / cost: V25 adds a table, GUC-reading triggers, and a synthetic "genesis" release backfill; pre-genesis supersession is flagged (best-effort NULL), not invented.
- Fitness: every published observation has a non-null `release_id`; every post-genesis `observation_revision` has a non-null `superseded_by_release_id` — SQL assertions in the migration test suite.

---

## Detailed Record (preserved verbatim from architect memory)

> Migrated from `.claude/agent-memory/architect/`. Backend/DB ADR family (see ADR-0023, ADR-016, ADR-017, ADR-018).
