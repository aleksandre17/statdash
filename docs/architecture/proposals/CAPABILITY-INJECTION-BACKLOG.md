...
- **R5 · Store views (concept note, owner instinct 2026-07-18 «ქვე-store»)** — a derived store as a SLICE/FREEZE of another (extends R4 extract + R1 remote): the flat-key namespacing (`gdp.regional`) stays convention-only per store-access.md's own refusal (no path resolution — Cube/Grafana consensus); the future word is a VIEW-store, not a sub-path. Anchor: Power BI composite models / DirectQuery-over-dataset · Tableau Extract. YAGNI-gated: builds only on a real journey ask. NOTE: the E-3 cascade itself is LIVE and upgraded (renderNode:252 — node storeKey › metric dataSource › page › default; the metric-knows-its-home semantic link is OUR beyond-class piece, SSOT-pinned by the 0083 fix).
