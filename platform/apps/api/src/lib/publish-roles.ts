...
//  (Protected Variations). pages.ts should then adopt this seam (SURFACED — it keeps
//  its own copy today; harmonizing is a follow-up, out of E0 scope).
//
 ...
 * pages.ts publish-gate shape).
 */
export function requirePublishRole(roles: string[] | undefined): void {
  const r = roles ?? []
  if (!PUBLISH_ROLES.some((role) => r.includes(role))) {
    throw new HttpError(403, 'admin role required to restore a revision')
  }
}
