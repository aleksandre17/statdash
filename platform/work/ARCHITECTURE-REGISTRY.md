...
| **AR-13** | **Theme-switcher as a config-rendered chrome node** | owner 2026-07-01: a dark theme exists but there is NO switcher UI, and theme-switching is NOT a first-class config node (locale IS — `LocaleSwitcherShell`). Build a `theme-switcher` chrome node mirroring the locale-switcher: config-driven, agnostic, toggles `[data-theme]` dark/light, persisted. Maximal-adoption (every chrome capability is a node) | (mirror locale-switcher) | VISION | build after the dark-mode theme hunt (aeb5cda) fixes theme coherence; then the switcher rides a clean theme system |
