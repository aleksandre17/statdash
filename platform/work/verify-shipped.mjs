// CAPTURE-ONLY probe of the just-shipped change on live prod (:3002).
// Handles 3 chart kinds: Apex bar/line (.apexcharts-canvas), custom SVG donut
// (.donut-legend + svg arcs + center text), div treemap (.chart-wrap innerText).
// Region selection via real gesture: locator.click on leaflet region paths.
...
const HELPERS = () => {
  ...
  window.__x.section = (id) => {
    ...
    if (apex) {
      ...
      // div-treemap or other custom: report tile-ish tokens
...
R.gdpYear = { kpis: await kpis(), structural: await sec('structural'), production: await sec('production'), income: await sec('income'), scan: await scan(), errors: [...errors] }
...
await settle(); await inject()
R.gdpRange = { toggledVia: toggled, noeShare: await sec('noe-share'), growthDynamics: await sec('growth-dynamics'), scan: await scan(), errors: [...errors] }
await page.evaluate(() => window.scrollTo(0, 0)); await page.waitForTimeout(400)
...
R.regionalA = { kpis: await kpis(), regionsBar: await sec('regions-bar'), sectors: await sec('sectors'), sectorsMulti: await sec('sectors-multi'), scan: await scan(), errors: [...errors] }
await page.evaluate(() => window.scrollTo(0, 0)); await page.waitForTimeout(400)
await page.screenshot({ path: OUT + '/regional-stateA.png', fullPage: true })

// 4: /ka/regional STATE B — select 2 regions via locator clicks on map paths
errors.length = 0
...
const mapHi = await page.evaluate(() => { const p = Array.from(document.querySelectorAll('.leaflet-container path.leaflet-interactive')); const sw = p.map(x => parseFloat(getComputedStyle(x).strokeWidth) || 0); const mx = Math.max(0, ...sw); return { total: p.length, maxStroke: mx, thickStroke: sw.filter(w => mx > 1.6 && w >= mx - 0.05).length, distinctStroke: [...new Set(sw.map(w => Math.round(w * 10) / 10))] } })
R.regionalB = { mapPaths: mapTotal, regionParam, selectedRegions: regionParam.split(',').filter(Boolean), registeredClicks: clicks,
  kpis: await kpis(), sectorsMulti: await sec('sectors-multi'), regionsBar: await sec('regions-bar'), mapHighlight: mapHi, scan: await scan(), errors: [...errors] }
await page.evaluate(() => window.scrollTo(0, 0)); await page.waitForTimeout(500)
await page.screenshot({ path: OUT + '/regional-stateB-2regions.png', fullPage: true })

// 5: /en/gdp locale spot-check
errors.length = 0
await page.goto(BASE + '/en/gdp', { waitUntil: 'networkidle', timeout: 40000 }).catch(e => errors.push('GOTO ' + e.message.split('\n')[0]))
await settle(); await inject()
R.enGdp = { structural: await sec('structural'), scan: await scan(), errors: [...errors] }
await page.evaluate(() => window.scrollTo(0, 0)); await page.waitForTimeout(400)
await page.screenshot({ path: OUT + '/en-gdp.png', fullPage: true })
