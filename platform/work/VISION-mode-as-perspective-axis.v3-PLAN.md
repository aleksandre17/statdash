...
**Residual the user must confirm (one item):**
1. **`scope.metric` SSOT name resolution.** P5 maps the year/range KPI measurement difference onto `PerspectiveDef.scope.metric` (a `MetricDef` ref). This assumes the range-perspective measurements (CAGR/share) are (or will be) registered `MetricDef`s in the semantic layer (R1, shipped). If any range measurement is **not** yet a named metric, P5 either (a) registers it as a `MetricDef` first, or (b) keeps that one node's measurement node-local via `value.type` (LOW-2 allows either; the FF forbids only *dual* encoding). Confirm: register-as-metric (cleaner, perspective-wide) vs node-local for the unregistered cases. *Lean: register-as-metric where the whole perspective shares it; node-local for one-offs.*
