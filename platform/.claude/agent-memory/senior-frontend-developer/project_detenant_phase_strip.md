...
- `i18n/feedback.ts` DECISION: KEEP — it backs generic engine/react components (EmptyState, ExportBar, PreliminaryBadge, SharePermalink) = UI chrome, NOT editorial content. But stripped the `ka` locale → en-only baseline; tenant locales arrive via manifest i18n.
