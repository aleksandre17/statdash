...
**Why:** The architecture-audit task assumed paths like `engine/plugins` and `apps/panel/src` with chart/kpi as `nodes`; reality is `engine/plugins/{nodes,panels}` and chart/table/kpi/gauge/map are **panels**, not nodes.
**How to apply:** Always ground path assumptions against this layout before grepping; the task-prompt paths are illustrative, not literal.
