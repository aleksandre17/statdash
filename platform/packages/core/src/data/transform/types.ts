...
export type TransformStep =
  ...
  /**
   * reduce — GROUP BY + collapse to one row per group. Cleaner API than
   * `aggregate`, with extended fn vocabulary (mean, first, last).
   *
   *   fn    — reduction function
   *   field — measure column to reduce
   ...
   *
   *   fn   — window function
   *   over — measure column to compute over
   *   by   — optional partition key
   *   n    — window size for movingAvg (default: 3)
   *   as   — output field name (default: `${over}_${fn}`)
   *
   * lag/diff: the output field is absent (not 0, not null) on the first
   * row of each partition — consistent with join's no-match behaviour.
   */
  | { op: 'window'; fn: 'movingAvg'|'cumSum'|'lag'|'diff'; over: string; by?: string; n?: number; as?: string }
  /**
   * joinByField — Hash join of two EngineRow sets on a shared key field.
   * Caller must resolve any DataSpec to rows before constructing this step.
   *
   *   by     — field name present on both sides
   *   mode   — join mode ('inner' | 'left' | 'outer')
   *   source — right-hand rows (already resolved)
   *
