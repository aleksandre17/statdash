  ...
  for (const cv of crossValidate) {
    ...
    if (!evalWhen(cv.check, vals)) {
      ...
      if (!perField[target] && !extra[target])
        // Validation messages are locale-neutral English fallbacks (see header);
        // flatten LocaleString to its English/first-available string at this boundary.
        extra[target] = resolveLocaleString(cv.message, 'en', 'en')
    }
  }
  return extra
}
