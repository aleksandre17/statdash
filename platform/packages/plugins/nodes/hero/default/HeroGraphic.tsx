...
export function HeroGraphic() {
  // SVG presentation attributes — var() is invalid here, so resolve the accent
  // tokens to their computed values at render (theme-aware via the cascade).
  const accent          = cssVar('--color-accent', '#0080BE')
  const accentSecondary = cssVar('--color-accent-secondary', '#00A896')
  return (
    <svg className="hero__graphic" viewBox="0 0 800 500" aria-hidden="true">
      ...
      <path
        ...
        fill="none" stroke={accentSecondary} strokeWidth="2"
      />
      {[100, 200, 300, 400, 500, 600, 700].map((x, i) => (
        <rect
          ...
          rx={4} fill={accent} opacity={0.04 + i * 0.008}
        />
      ))}
    </svg>
  )
