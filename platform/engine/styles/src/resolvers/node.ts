...
// Resolve a StyleValue-typed responsive prop to a CSS string (number → px).
function valStyle(val?: ResponsiveVal<StyleValue>): string | undefined {
  return dimensionStyle(val)
}

// Resolve a plain responsive prop (string / number) to a CSS string as-is.
function plainStyle(val?: ResponsiveVal<string | number>): string | undefined {
  const v = resolveResponsive(val).default
  if (v === undefined) return undefined
  return String(v)
}
