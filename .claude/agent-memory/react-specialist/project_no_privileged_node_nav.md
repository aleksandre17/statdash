...
The No-Privileged-Node fix landed (mirrors the presentation-projector registry [[project-presentation-registry]]). `navUtils.ts` is now a GENERIC registry visitor — ZERO `type === 'section'/'geograph'/'row'` literals.
...
**Plugin META declarations (one line each):** section + geograph `caps:[…,'nav-contributor']`; row `caps:['nav-transparent']`. They use the default descriptor (no explicit `navContribution`).
