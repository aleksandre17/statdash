...
**Core files created:**
- `packages/react/src/engine/extensions/ExtensionPoint.ts` — typed slot class, identity by string id (HMR-safe)
- `packages/react/src/engine/extensions/ExtensionRegistry.ts` — contribute/resolve with order + when filter
- `packages/react/src/engine/extensions/useExtensions.ts` — React hook wrapping resolve()
- `packages/react/src/engine/extensions/points.ts` — PANEL_TITLE_BADGE + SECTION_HEADER_ACTIONS canonical points
