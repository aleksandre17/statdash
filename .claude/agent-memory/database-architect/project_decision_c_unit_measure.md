...
**How to apply:** Read-surface for the cube-profile endpoint is `stats.measure_unit_resolved`. The measure↔dataset bridge is `stats.dataset_dimension` (classifiers are dataset-agnostic; a measure classifier is shared across every dataset declaring the measure axis — the within-measure cartesian in V21's join is intentional). Legacy unit code normalization in the backfill: `'PCT' -> 'PERCENT'`.

**V20/V21 backfill fail-fast (operational gotcha):** V20's backfill `UPDATE stats.classifier SET unit_code = metadata->>'unit_measure'` resolves against the FK to `stats.unit_measure(code)`. If a measure carries a legacy code that is neither `PCT` (→`PERCENT`) nor present in the V16 seed, the FK rejects the UPDATE and **aborts the whole migration** — fail-fast by design (a halt is recoverable; a silently-wrong unit renders GEL 12,000 next to GEL 12,000,000,000). Before applying V20 to the live volume, confirm the V16 `stats.unit_measure` seed covers every in-use `metadata->>'unit_measure'` value. The backfill is idempotent (`WHERE unit_code IS NULL`), so a fixed seed + re-apply converges.
