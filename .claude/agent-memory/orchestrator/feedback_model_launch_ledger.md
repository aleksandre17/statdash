...
4. **GAP CLOSED (2026-07-08, lead-built):** `kit/hooks/subagent-ledger.py` on SubagentStop now writes a RUN line per finished agent automatically — `model-actual` + `out-tokens` parsed from the agent's own transcript (`~/.claude/projects/<slug>/<session>/subagents/agent-*.jsonl` — note the subagents/ subdir), deduped by run id, with `⚠ MODEL-VERIFY` when a run's family was never LAUNCH-requested that day. Requested (LAUNCH, lead-written) vs verified (RUN, hook-written) now sit side by side in token-log.md.
