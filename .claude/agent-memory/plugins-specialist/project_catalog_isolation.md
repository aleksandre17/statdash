...
**Pattern:** Each `{plugin}/default/meta.ts` holds only the META constant — imports `type NodeSliceMeta/PanelSliceMeta/PageSliceMeta` from `@statdash/react/engine` (formerly `@geostat/react/engine`) and value imports from the co-located `*Node.ts` (Zod schemas, defaults, slots, groups). Zero React/TSX imports.
...
`apps/panel/src/platform-capabilities.ts` now imports `PLUGIN_CATALOG` from `@statdash/plugins/catalog` and exposes it as `PLATFORM_CAPABILITIES.nodes`.
...
**How to apply:** Any future plugin (node/panel/page) must follow the same split: `meta.ts` (pure TS) + `index.ts` (re-exports META from meta.ts + all Shell/type exports). Never inline META in index.ts. Related architect note: [[project_catalog_react_purity]].
