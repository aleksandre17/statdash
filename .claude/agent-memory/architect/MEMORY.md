...
- [Classifier code-path ADR](project_classifier_code_path_adr.md) — ADR-0023: classifier hierarchy moves surrogate-id-chain → code-chain LTREE over (dim_code,code); V23/V24/V25 Strangler-Fig (not migrated: <8KB)
- [Vintage-as-Release ADR](project_vintage_release_adr.md) — ADR-0025 SDMX-P0-2: release = publication-event aggregate stamped on observation + observation_revision; V25 additive (not migrated: <8KB)
- [Bootstrap Runner ADR](project_bootstrap_runner_adr.md) — ADR-0026 (Phase A): gut apps/geostat into a generic SDUI runner booting from GET /api/bootstrap; store half already live (not migrated: <8KB; Phases B/C = ADR-018/ADR-017)
