...
**Why:** Public API changes to `@statdash/engine` (formerly `@geostat/engine`) are cross-layer contracts. Downstream consumers (react layer, plugins, app) depend on them. The hook is a mandatory gate — skipping it or deferring it to after the edit defeats the purpose.
