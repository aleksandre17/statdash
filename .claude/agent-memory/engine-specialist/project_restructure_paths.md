...
- Per-package vitest aliases map `@geostat/react` → `packages/react/src`, so deep subpath imports like `@geostat/react/engine/slice-meta` resolve in tests even though package.json `exports` only lists `.` and `./engine`.
- Pre-existing TS debt (do not treat as regressions): `erasableSyntaxOnly` TS1294 errors in core/react under `tsc -b` strict invocation; `leaflet` missing types in plugins georgraph; `process` in expr/derive. Logged in [[project_debt]].
