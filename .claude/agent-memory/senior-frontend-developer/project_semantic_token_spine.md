...
- **Tier 2 (semantic roles)**, same `:root`, brand-NEUTRAL defaults: text/surface/border/accent
  role families, chart-frame/-grid, trend {positive,negative}, heading-display, error family,
  STATUS families (`--status-*` alert tones AND the DISTINCT SDMX
  `--status-obs-{preliminary,estimate,revised,confidential}-*`+`--status-total-*` taxonomy — these
  do NOT share values with `--status-*`, a different concern). All mirrored into `tokens/color.ts`
  + user-facing ones into `catalog/color.ts` (bilingual).
...
- **FF-THEME-COMPLETE** (`tokens.parity.test.ts`) — every `var(--color-*)` referenced anywhere in
  ...
  layer fails.
- **FF-TOKEN-ONLY** (`token-cohesion.fitness.test.ts`) — scans `packages/{plugins,react,charts,
  core}/src/**` (.css/.ts/.tsx) for hex/rgb/hsl; **ERROR**, not warn. Strips `cssVar(...)`/
  `var(--)`, rgba-overlays, and comments before scanning. The allowlist is **value-aware**: an
  entry may pin `literals:[…]` so ONLY those exact values are exempt in that file (a brand hex
  slipped in beside a sanctioned seed still fails). 4 entries max, each asserted still-present:
  `mapColorUtils` DEFAULT_PALETTE (Leaflet ColorBrewer-Blues) + `PropSchemaForm` whole-file
  `#000000` + `charts/colors.ts` neutral series-grey/action-red wire-seeds +
  `core/registry/resolvers.ts` growth +/- semantic pair. The `charts`+`core` scan scope closed a
  real hole: `charts/colors.ts DEFAULT_ACCENT_COLOR` had shipped as the exact tenant brand hex
  under a "neutral default" guise, unscanned until then.
...
## The byte-identity gotcha (governs any future tokenization pass)

Tokenizing shells against the semantic spine under a **byte-identical** invariant has systematic
traps the ADR's role→value table hides:

**1. A "collapse many literals to one role" pass is a ΔE<3 MERGE, not byte-identical** — only the
canonical resolved token value renders identically; divergent near-duplicate hexes must stay
literal + get reported, not silently tokenized, until a ΔE trade is explicitly ratified (as
happened at Pfinal below — the SPECIFIC hex-by-hex list from that hold is no longer current, since
Pfinal folded them all in; the PRINCIPLE — don't silently ΔE-merge without ratifying it — still
applies to any FUTURE tokenization pass).
...
Pfinal is the point where the byte-identity hold above was explicitly LIFTED for a cohesion-first
trade — the divergent near-duplicate literals (text-muted/-secondary/-faint, surface-raised/
-sunken/-frame, border-strong, accent-muted families) were folded into their nearest existing role
(ΔE 3–10, same design intent). There is NO allowlist for frozen color debt in the FF-TOKEN-ONLY
error gate — a straggler literal must either collapse into an existing role or get its own new
role (trend/error/heading-display/etc., listed above).

**Geograph straggler (closed):** `GeoMap.tsx` `FILL_COLOR`/`#fff` → `cssVar('--color-accent',...)`/
`cssVar('--color-surface',...)` (Leaflet `PathOptions` is JS-fed, `var()` invalid — gotcha #2).

**Charts EMIT path — the charts-LOCAL cssVar twin.** The server-side SVG realizer
(`packages/charts/src/emit/`) had baked hex literals — FF-TOKEN-ONLY scans `charts/src/**`.
`charts` MUST NOT import `@statdash/styles` (Law 3, inside the arrow `…←core←charts←react`).
Lawful path = a deliberate charts-local twin `emit/cssVar.ts` (byte-identical logic to styles'
cssVar, `getComputedStyle`→fallback): the VALUE SSOT stays single (tokens.css), only the ~5-line
resolver duplicates across the arrow boundary. The emitter is server-side (no DOM) → cssVar
returns the fallback = deterministic export; a browser export re-themes for free.

See also [[project_dark_mode_theming]] (the dark-mode override LAYER built on top of this spine,
plus a tenant stylesheet silently defeating the dark-flip mechanism by source-order).
