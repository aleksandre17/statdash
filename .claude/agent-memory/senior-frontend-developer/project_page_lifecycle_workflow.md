...
**Palette completeness is architectural (OCP), not curated:** `getPaletteEntries()` derives from `nodeRegistry.list()`; `setupCanvasRegistry` registers all `@plugins/{panels,nodes,...}` via Object.values. text+gauge appear automatically because they're now in the `packages/plugins/panels/index.ts` barrel (the plugins barrel export gap this used to flag is fixed).
