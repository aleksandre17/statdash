...
> **Generic + domain-free** — load on demand (not in every prompt). The architect *selects what fits this project's stack*; it does not apply everything. For every significant choice, **name the trade-off** and prefer encoding the resulting rule as a fitness function (`08`/`10`).
...
3. **Harden the choice** — turn the resulting invariant into a fitness function (`law_patterns` hook / ArchUnit), not a comment (`10`).
