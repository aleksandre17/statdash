...
2c. **0099** — THE CROSS-GESTURE LANDS IN THE GRID, not JSON (0089 finding #2): «დაათვალიერე workbench-ში» seeds a pipeline spec that `DataSpecEditor` routes to JsonFallback — the owner asks to browse and gets raw JSON. Land on the WORKBENCH grid (unification law). **HOT** — defeats 0091's whole point.
