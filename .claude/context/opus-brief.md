...
- **▶ STEP 1 (next work session): live-verify `8abb6b8d`** — capability restoration BUILT+GATED (R1–R6 back: dedicated editors un-diverted, `WorkbenchAdvanced` = SpecTypePicker + AdvancedRawPanel + ReadOnlyJson, FF-EDITOR-CAPABILITY-PARITY reds regressions). NOT yet deployed/walked — walk the 5 capabilities on :3013 per the commit body, then close.
