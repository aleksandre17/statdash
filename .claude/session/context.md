...
- **Wave-1 (IN FLIGHT a268, charts lane):** charts/src/colors.ts `DEFAULT_ACCENT='#0080BE'`(=geostat brand) → neutral + extend FF-TOKEN-ONLY guard to charts/core (the systemic guard hole). Commit on return.
- **Wave-2 (IN FLIGHT a17c, styles/app/kpi lane):** de-brand FONT axis → new `--font-family-display` token (9 shared plugin CSS hardcode 'Noto Serif Georgian'/'BPG Arial'); DELETE dead `.sc` (card.css) + app `@layer components`+fadeInUp; **kpi-strip horizontal-padding flush** (owner-caught double-inset misalignment). Commit on return.
