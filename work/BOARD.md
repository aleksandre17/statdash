...
**Gaps — REAL remaining (G1/G2/G4/G5/G6 all CLOSED; board was stale):** ✅ nested-prop writes — Inspector now WRITES dotted-path fields via immutable `setAtPath` (dual of `getAtPath` in `showWhen.ts`), wired into `patchProp` + `updateChromeConfigPatch`; immutability + round-trip + undo/redo locked (125 tests). ⏳ **Locale SSOT** (`inspector/useActiveLocales.ts` hardcodes `['ka','en']`) — DEFERRED: active-locale *set* isn't client-side; needs server to expose the V13/V14 `config.locale` list on the site-config payload (`SiteConfigMap.activeLocales` → `SiteDef`), then `useActiveLocales` is the one swap point. Blocks I18N-4. ⏳ **G3 live-data preview** — `CanvasView` renders an empty `staticStore`; genuinely open, lower leverage.
