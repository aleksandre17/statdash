...
## 🎯 RENDER-PIPELINE TARGET epic (items 0009–0042) → **`work/board/render-pipeline.md`**

> This epic now has its OWN board (one-concern-per-file): **[`work/board/render-pipeline.md`](board/render-pipeline.md)** is the single source for the render-pipeline target (decisions O-1…O-12, capabilities C1–C7, elements E1–E9, FF suite, LV live-verify checks). SSOT specs: `platform/work/SPEC-render-pipeline-target.md` + `platform/work/SPEC-render-pipeline-target.DELTA-6-14.md`.
...
| I18N-4 | Frontend locale SSOT | ✅ | Old `REQUIRED_LOCALES`/`src/config/locales.ts`/`locale-coverage.test` artifacts no longer exist (refactored away). Panel locale SSOT now wired: `GET /api/config/site` projects `activeLocales` from `config.locale`; `useActiveLocales` reads it (graceful fallback). |
...
**Status:** Phases 0-4+6 ✅ DONE (783 tests green, arrow proven enforcing, laws reconciled). **Phase 5 (scope rename) — DECIDED GO** (lead, senior call: first-tenant erosion + pre-publish cheapest window + de-tenanting north-star). **Sequenced: execute as ONE atomic sweep when Constructor lanes A+B quiesce green** (rename touches every package.json name + import — can't run concurrent with active packages/+panel edits). Then green-gate + fitness ("no `@geostat/<lib>` survives") + resume C3/C5. ✅ **Phase 5 DONE (verified 2026-06-24):** every package is `@statdash/*` (api/panel/charts/contracts/engine/expr/plugins/react/styles), zero `@geostat/*` names survive, `apps/geostat`=`national-accounts` (tenant), locked by `tests/no-geostat-scope.fitness.test.ts` (green in the 933 suite). Build GREEN. Prior "HELD for GO" note was stale.

---

## Constructor (Phase 2) — Architecture (proposed)

Status: **BUILT & GREEN (verified 2026-06-24 recon — prior "PROPOSED/design-only" was fiction).** `apps/panel` ships: schema-driven Inspector (mounted, reads `getSchema` → `FieldControlRegistry` + `LocaleField` + `EnumRefField`), unified `CanvasNode {type,variant,props,childIds}` store (enum deleted), `to/fromNodePageConfig` lossless round-trip, cube-profile-gated palette + `suggestPanels`, publish/version/RBAC UI, undo/redo. **125/125 panel tests, build:panel green.** ADR + fitness fns: architect memory `adr_constructor_phase2.md`. North star: "Constructor generates JSON, no code."
...
**Gaps — REAL remaining (G1/G2/G4/G5/G6 all CLOSED; board was stale):** ✅ nested-prop writes — Inspector now WRITES dotted-path fields via immutable `setAtPath` (dual of `getAtPath` in `showWhen.ts`), wired into `patchProp` + `updateChromeConfigPatch`; immutability + round-trip + undo/redo locked (125 tests). ✅ **Locale SSOT (I18N-4 CLOSED):** `GET /api/config/site` now projects `activeLocales`/`defaultLocale` live from `config.locale` (SSOT, not duplicated into the blob); shared `SiteConfigResponse` in `@statdash/contracts`; `fromApiSite`→`SiteDef.activeLocales`→`useActiveLocales` (graceful degrade: site set → `[defaultLocale]` → `['ka','en']`). Follow-on (noted, deferred): `api-actions.ts` save-guard still derives from `defaultLocale` not `activeLocales` — wire when a non-ka/en locale ships. ✅ **G3 live-data preview (G3.0+G3.1 SHIPPED):** stats store-builder promoted to shared `@statdash/plugins/datasources` (SSOT, runner byte-identical); canvas `structural|live` toggle injects a live store via the runner's `buildStoreManifest` seam (zero engine change); fail-soft to `staticStore` + badge; structural default (opt-in live, first-cube-bound-wins). ⏳ G3.2 (debounce DataSpec edits + request-volume hardening) deferred.
