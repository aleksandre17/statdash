...
const OUT = new URL('./portal-walk/', import.meta.url).pathname.replace(/^\/([A-Za-z]:)/, '$1')
...
await page.goto(BASE + '/gdp'); await settle()
...
const years = xcats.filter(t => /^20\d\d$/.test(t.trim()))
