...
depends_on: ["0016", "0017", "0020", "0036"]
links:
  - platform/work/SPEC-render-pipeline-target.md
  - platform/work/SPEC-render-pipeline-target.DELTA-6-14.md
---
**Goal** — Real GDP growth bars (% axis, negatives below zero), contribution-to-growth bars (no clipped labels), and a per-capita line where 2014 is NOT 483; plus C7 table views (img_8: real-growth year│%, share table) that re-encode the same warmed rows.

**Implements** — SPEC §2 E7 + §3.

**Files / modules touched** — GDP dynamics chart config (real-growth bar, contribution bar, per-capita line).

**Dependencies** — 0016 (C1), 0017 (C2), 0020 (C6-d — per-capita row-selection), 0036 (C7 — the chart↔table dual-view; img_8 shows these sections as tables). Note: the `mean` of the real-growth series feeds the E1 avg-real-growth KPI (0019/0022), not this chart directly.
...
- [ ] C7 table views (0036) re-encode the same warmed rows (img_8: real-growth year│%, share 23.1%→12.4%); FF-DUALVIEW-ONE-DATA green.
