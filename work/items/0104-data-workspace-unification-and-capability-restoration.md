...
1. **Restore capability — BUILT + GATED `8abb6b8d` (live-verify = next-session STEP 1, deliberately not rushed at session end).** Narrowed the fold gate → timeseries/growth/pivot/transform back to their full dedicated editors (R2/R3/R4/R5); `WorkbenchAdvanced.tsx` = SpecTypePicker (R1, owned Radix) + AdvancedRawPanel (encoding/measure/filter/wells + writable raw JSON = the owner's complaint) + ReadOnlyJson (R6); new `FF-EDITOR-CAPABILITY-PARITY` gate reds the build on any future silent regression. 1275 panel tests green, tsc clean. NOT deployed — walk the 5 capabilities live on :3013 (see the commit body) to close it. This partially reverts Step A's three-pane VIEW for those 4 kinds (correct — it was read-only/lossy).
