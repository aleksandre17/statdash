...
## ✅ CLOSED (2026-07-23 night — every residual root-fixed + live-proven)
- **R1** `b3fb28d0`+`5de4b660`+`134bef12`: ctx SSOT (`filterCtxCore`, provider-free zoom, Tier-3 via page store) + the TRUE root — preview store cascade lacked the PAGE-storeKey tier (debugger wire-proof: wildcard query read the blind first store REGIONAL_GVA). LIVE: 5 real rows at the Get step.
- **R2** `b3fb28d0`: one `facetAppliesToElement` predicate feeds BOTH inspector surfaces (the lying "SAME derivation" comment killed). LIVE: section Data facet present.
- **R3** `bf737eb5`: stats-api last raw fetch → seam; config/bootstrap = unrouted panel paths (handoff designed, queued); cube-profile 2× = legitimate two-phase owners (ETag api follow-up queued).
- **R4** → executed as ONE-PIPE U0 `ee436398` + structural-coincidence `3af53567` + **U3 corpus rewrite `930bebe9`** — stored ≡ canonical now; the pane shows the stored artifact + the distinct ObsQuery wire-truth only (no phantom duplicate; 61-one-block.png).
- **R5** `b3fb28d0`+`e18c5b6c`: select-behind + ascent-from-selection + parents recorded BY the framing recursion (rendered-tree SSOT, synth ids). LIVE: monotonic chart→wrapper→section→grid→inner-page.
